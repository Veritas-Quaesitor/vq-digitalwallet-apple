...
(function () {
  ...
  (function (global, factory) {
     ...
     * @readonly
     */
    var VALID_CARD_NETWORKS = ["amex", "chinaUnionPay", "discover", "eftpos", "electron", "elo", "interac", "jcb", "mada", "maestro", "masterCard", "mir", "privateLabel", "visa", "vPay"];
    ...
    function generateUUID() {
      ...
      throw new Error("crypto API unavailable. A secure random number generator is required. " + "Use a modern browser or provide a crypto polyfill.");
    }
    ...
    VqDigitalWalletApple.prototype = {
      ...
      _validateConfig: function () {
        ...
        } else if (!/^merchant\.[a-zA-Z0-9]([a-zA-Z0-9\-.]*[a-zA-Z0-9])?$/.test(config.merchantIdentifier)) {
          errors.push("merchantIdentifier must follow the format merchant.com.yourcompany");
        }
        ...
        if (!Array.isArray(config.allowedCardNetworks) || config.allowedCardNetworks.length === 0) {
          ...
          if (invalidNetworks.length > 0) {
            errors.push("allowedCardNetworks contains unsupported networks: " + invalidNetworks.join(", "));
          }
        }
        ...
        if (!Array.isArray(config.merchantCapabilities) || config.merchantCapabilities.length === 0) {
          ...
          var invalidCapabilities = config.merchantCapabilities.filter(function (c) {
            return VALID_MERCHANT_CAPABILITIES.indexOf(c) === -1;
          });
          if (invalidCapabilities.length > 0) {
            errors.push("merchantCapabilities contains unsupported values: " + invalidCapabilities.join(", "));
          }
        }
        ...
        if (validButtonStyles.indexOf(config.buttonStyle) === -1) {
          errors.push("Invalid buttonStyle '" + config.buttonStyle + "'. Must be one of: " + validButtonStyles.join(', '));
        }

        // Validate buttonType
        var validButtonTypes = ['plain', 'buy', 'pay', 'order', 'donate', 'subscribe', 'checkout', 'book', 'add-money', 'contribute', 'reload', 'rent', 'save', 'tip', 'top-up'];
        if (validButtonTypes.indexOf(config.buttonType) === -1) {
          errors.push("Invalid buttonType '" + config.buttonType + "'. Must be one of: " + validButtonTypes.join(', '));
        }
          ...
          return request.canMakePayment().then(function (canPay) {
            ...
            if (self.config.mode === "development") {
              if (canPay) {
                ...
                console.warn("[VqDigitalWalletApple] Apple Pay not available on this device");
              }
            }
            return canPay;
          }).catch(function (err) {
      ...
      _validatePaymentData: function (paymentData) {
        ...
        if (paymentData.description !== undefined) {
          try {
            paymentData.description = sanitizeString(paymentData.description, 255, "description");
          } catch (e) {
            errors.push(e.message);
          }
        }
      ...
      requestPayment: function (paymentData) {
        ...
        try {
          ...
          var timeoutId = setTimeout(function () {
            ...
            if (self._currentPaymentRequest && typeof self._currentPaymentRequest.abort === "function") {
              try {
      ...
      _handlePaymentMethodChange: function (event) {
        if (this.config.mode === "development") {
          console.log("[VqDigitalWalletApple] Payment method changed:", event.methodName);
        }
     ...
     * @readonly
     */
    VqDigitalWalletApple.defaults = Object.freeze ? Object.freeze(extendDeep({}, DEFAULTS)) : extendDeep({}, DEFAULTS);
     ...
     * @readonly
     */
    VqDigitalWalletApple.ERROR_CODES = Object.freeze ? Object.freeze(ERROR_CODES) : ERROR_CODES;
